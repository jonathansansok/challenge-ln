import typeCard from "./cardStyles";

const notas4ej1 = [
  {
    author: "Jonathan Sansó",
    imageAlt: "Descrip. de imagen",
    imageAuthor: "images/avatar.png",
    imageNote: "https://www.lanacion.com.ar/resizer/v2/la-isla-privada-que-se-compro-patrick-parker-TASE4GRYU5GMHISOHG55J6TDQM.JPG?auth=51165c311f43cafebb8e303669bcbd65f1c50437c4e43550ef552bcdd683dbe1&width=880&height=586&quality=70&smart=true",
    lead: "Noviembre y lluvia",
    marquee: "mismo autor",
    title:
      "Expertos advierten que es normal en el hemisferio sur",
    subTitle: "Efectos colaterales positivos en el agro y consumo de electricidad, leer mas...",
    type: typeCard.note,
    urlNote: "https://www.smn.gob.ar/",
  },
  {
    author: "María Salva",
    imageAlt: "Descrip. de imagen",
    imageAuthor: "",
    imageNote: "https://resizer.glanacion.com/resizer/v2/eeuu-covid-19coronavirus-mundo-new-york-nueva-L2SNGRTR6VEQ5JNR44VDQ5IJVA.JPG?auth=28ed408b3509ec651a71854af59f21d035de5aa7e21e5549c91c17db85bf59d6&width=780&height=520&quality=70&smart=true",
    lead: "Noviembre y lluvia",
    marquee: "mismo autor",
    title:
      "Expertos advierten que es normal en el hemisferio sur",
    subTitle: "Efectos colaterales positivos en el agro y consumo de electricidad, leer mas...",
    type: typeCard.note,
    urlNote: "www.nota.com.ar",
  },
  {
    author: "Camila Isla",
    imageAlt: "Descrip. de imagen",
    imageAuthor: "",
    imageNote: "https://resizer.glanacion.com/resizer/v2/el-uso-de-bicicletas-o-monopatines-electricos-ZXM5M5UQSJBHVEY7QS2ZGHOWZU.JPG?auth=92f0f7378d541c377be584194e941fe0ee33320046604e47562a7c8f6eb88b1f&width=780&height=520&quality=70&smart=true",
    lead: "Noviembre y lluvia",
    marquee: "mismo autor",
    title:
      "Expertos advierten que es normal en el hemisferio sur",
    subTitle: "Efectos colaterales positivos en el agro y consumo de electricidad, leer mas...",
    type: typeCard.note,
    urlNote: "https://www.lanacion.com.ar/propiedades/inmuebles-comerciales/la-vuelta-a-la-oficina-es-obligada-o-por-eleccion-como-tomar-la-decision-y-que-factores-influyen-nid10112023/",
  },
  {
    author: "Juan Areta",
    imageAlt: "Descrip. de imagen",
    imageAuthor: "",
    imageNote: "https://resizer.glanacion.com/resizer/v2/lamborghini-urus-6SIPOLUIM5CIJGQKXQDYD7FVJA.jpg?auth=d3d3d539816a19c1d6f0736ae9602b54bf65f924b673025a881edd84d78c3d3e&width=780&height=520&quality=70&smart=true",
    lead: "Noviembre y lluvia",
    marquee: "mismo autor",
    title:
      "Expertos advierten que es normal en el hemisferio sur",
    subTitle: "Efectos colaterales positivos en el agro y consumo de electricidad, leer mas...",
    type: typeCard.note,
    urlNote: "www.nota.com.ar",
  },
];



const card3 = [
  {
    author: "Pedro Villa",
    imageAlt: "Descrip. imagen",
    imageNote: "https://www.lanacion.com.ar/resizer/v2/escapada-visitar-los-caracteristicos-paredones-45LBYAO5PVBUZIALKWDJAVH7IE.jpg?auth=1bd21b7995cc428b0a8d58edc002e4d42ba622154a113ee8ef366f64b6194c53&width=1920&height=1280&quality=70&smart=true",
    lead: "Lluvia en vacaciones.",
    title:
      "La solución",
    subTitle: "Lanzan una App para aprovechar los días de lluvia en cualquier parte del país.",
    type: typeCard.note,
    urlNote: "  ",
  },

  {
    author: "Augusto Kansas",
    imageAlt: "Descrip. imagen",
    imageNote: "https://www.lanacion.com.ar/resizer/YsCXG3QLSZ2hGHxdaqENlf35HkQ=/610x406/smart/filters:format(webp):quality(70)/cloudfront-us-east-1.images.arcpublishing.com/lanacionar/HQWZQN5XYNBJTKN2FYJSRW4D24.jpg",
    lead: "Al Norte",
    title:
      "Turismo en USA o Canadá",
    subTitle: "Se filtraron archivos sobre como atraen turistas y compiten",
    type: typeCard.note,
    urlNote: "  ",
    
  },
  {
    author: "Juan Blas",
    imageAlt: "Descrip. imagen",
    imageAuthor: "images/avatar.png",
    lead: "Europa o Caribe.",
    marquee: "",
    title: "Nueva comparativa de precios en vuelos, hospedajes y costos de vida",
    subTitle: "",
    type: typeCard.author,
    urlNote: "www.depegar.com",
  },
];

const focalIzquierdoData = [
  {
    author: "Jonathan Sansó",
    imageAlt: "Descrip. imagen",
    imageAuthor: "",
    imageNote: "https://www.lanacion.com.ar/resizer/v2/el-onepus-12-ya-fue-presentado-en-china-y-llegara-AEWZJYM5V5BHVHY7GFQYSCRGRY.jpg?auth=be62dfbe9fbeb526a78ff6e3e228d78422f69eb393f9443bbc8eef9a8a7538b0&width=880&height=586&quality=70&smart=true",
    lead: "DIGITALIZARSE.",
    title:
      "Masiva apertura en Twitch y Youtube de los clásicos periodistas de TV.",
    subTitle:
      "En todo el país comunican lo mismo pero replicado en plataformas digitales. La TV hace simbiosis con Android/Apple para conocer noticias rápidamente.",
    type: typeCard.note,
    urlNote: "www.youtube.com.ar",
  },
  {
    author: "Juan Bugera",
    imageAlt: "Descrip. imagen",
    imageAuthor: "",
    imageNote: "https://resizer.glanacion.com/resizer/v2/el-portugues-da-silva-un-emblema-de-la-grabacion-QDP5GDVWD5GBTILBFMUJX5W6HM.JPG?auth=d302fe9367534b109652059947987914d9edb2303058b750115277b040255308&width=780&height=520&quality=70&smart=true",
    lead: "El sonido digital busca lo analógico",
    title:
      "Software para lograr emulación analógica",
    subTitle: "",
    type: typeCard.note,
    urlNote: "https://slatedigital.com/",
  },

  {
    author: "Pablo Waves",
    imageAlt: "Descrip. imagen",
    imageAuthor: "",
    imageNote: "https://www.lanacion.com.ar/resizer/v2/la-nueva-funcion-de-apple-namedrop-alerto-a-las-D3RT5TQ2VZE3DNF7UTG7M7TBWE.jpg?auth=60e5b106cbd07ef252404eadb960f9ceef73bccba2e0a53304a4000aaf36a5f3&width=880&height=586&quality=70&smart=true",
    lead: "Windows Multimedia.",
    title:
      "la gran marca prevee equiparar a Apple en calidad de sonido para 2026",
    subTitle: "",
    type: typeCard.note,
    urlNote: "https://apogeedigital.com/",
  },
];

export { focalIzquierdoData, notas4ej1, card3 };
